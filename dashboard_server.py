...
usage from all AI tools: Claude Code, OpenCode, 9router, Hermes, Codex CLI,
...
FREE_MODELS = {
    "nvidia", "openrouter", "nemotron", "glm", "deepseek-v4-flash",
    "laguna", "gpt-oss", "qwen3", "gemma", "big-pickle",
...
def get_pricing(model: str):
    ...
    if "deepseek" in m and "r1" in m:
        return PRICING["deepseek-r1"]
    if "deepseek" in m:
        return PRICING["deepseek-v3"]
    return None
...
def get_dashboard_data(db_path: Path = DB_PATH) -> dict:
    ...
    """).fetchall()
    all_models = [{"source": r["source"], "model": r["model"]} for r in model_rows]
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/rescan":
            threading.Thread(target=scan, kwargs={"verbose": False}, daemon=True).start()
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            self.end_headers()
            self.wfile.write(json.dumps({"status": "scanning"}).encode())
            return
...
function renderSessions(){
  ...
  body.innerHTML=shown.map(r=>{
    ...
    const topic=r.topic||'<span class="muted">untitled</span>';
    return `<tr>
